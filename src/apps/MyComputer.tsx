    ...
    <div className="flex flex-col h-full bg-[#c0c0c0]">
      ...
      <div className="flex-1 overflow-auto p-3 space-y-3">
        ...
        <div className="win98-border-raised p-2">
          ...
          {drives.map((drive, i) => {
            const DriveIcon = (Icons as any)[drive.icon] || Icons.HardDrive;
            return (
              <div key={i} className="flex items-center gap-3 mb-2 last:mb-0">
                <DriveIcon size={32} className="text-[#808080] shrink-0" />
                <div className="flex-1">
                  <div className="text-[11px] font-bold">{drive.name}</div>
                  <div className="win98-progress w-full mt-1">
                    <div
                      className="h-full"
                      style={{
                        width: `${drive.percent}%`,
                        backgroundColor: drive.percent > 80 ? '#ff0000' : '#000080',
                      }}
                    />
                  </div>
                  <div className="text-[10px] text-[#808080] mt-[2px]">
                    {drive.used} used / {drive.total} total ({drive.free} free)
                  </div>
                </div>
              </div>
            );
            ...
            <tbody>
              {systemInfo.map((info, i) => (
                <tr key={i} className={i % 2 === 0 ? 'bg-white' : 'bg-[#f0f0f0]'}>
                  <td className="px-2 py-[3px] font-bold w-[140px] border border-[#dfdfdf]">{info.label}</td>
                  <td className="px-2 py-[3px] border border-[#dfdfdf]">{info.value}</td>
                </tr>
              ))}
            </tbody>
          </table>
