...
export default function ColorPicker() {
  ...
  const [savedColors, setSavedColors] = useState<string[]>([
    '#008080', '#000080', '#2d5a27', '#c6a84b', '#1a7a6d', '#c0c0c0',
  ]);
  ...
  return (
    <div className="flex flex-col h-full bg-[#c0c0c0] p-2">
      ...
      <div className="flex gap-2 mb-2">
        ...
        <div className="flex-1">
          <div className="text-[11px] mb-1">
            ...
            <span className="font-mono cursor-pointer hover:underline" onClick={() => handleCopy(hex)}>{hex}</span>
          </div>
          <div className="text-[11px] mb-1">
            <span className="font-bold">RGB:</span>{' '}
            <span className="font-mono cursor-pointer hover:underline" onClick={() => handleCopy(`rgb(${rgb[0]}, ${rgb[1]}, ${rgb[2]})`)}>
              {rgb[0]}, {rgb[1]}, {rgb[2]}
            </span>
          </div>
          <div className="text-[11px] mb-1">
            <span className="font-bold">HSL:</span>{' '}
            <span className="font-mono cursor-pointer hover:underline" onClick={() => handleCopy(`hsl(${hue}, ${sat}%, ${light}%)`)}>
              {hue}, {sat}%, {light}%
            </span>
          </div>
          <div className="text-[10px] text-[#808080]">(Click values to copy)</div>
        </div>
      </div>
      ...
      <div className="space-y-2 mb-3">
        <div>
          ...
          <div className="win98-border-field p-[2px]">
            <div className="h-[16px] relative" style={{
      ...
      {/* Save button */}
      <button className="win98-btn text-[11px] mb-2 self-start" onClick={handleSave}>
