...
export default function Clock() {
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < 12; i++) {
      ...
      ctx.fillText(
        String(num),
        center + numRadius * Math.cos(angle),
        center + numRadius * Math.sin(angle)
      )
    }
    ...
    ctx.lineTo(
      center + radius * 0.5 * Math.cos(hAngle),
      center + radius * 0.5 * Math.sin(hAngle)
    )
    ctx.strokeStyle = '#000'
    ctx.lineWidth = 3
    ctx.lineCap = 'round'
    ...
    ctx.moveTo(center, center)
    ctx.lineTo(
      center + radius * 0.7 * Math.cos(mAngle),
      center + radius * 0.7 * Math.sin(mAngle)
    )
    ctx.strokeStyle = '#000'
    ctx.lineWidth = 2
    ctx.lineCap = 'round'
    ...
    ctx.moveTo(center, center)
    ctx.lineTo(
      center + radius * 0.75 * Math.cos(sAngle),
      center + radius * 0.75 * Math.sin(sAngle)
    )
