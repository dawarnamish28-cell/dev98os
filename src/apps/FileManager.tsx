  ...
  return (
    <div className="flex flex-col h-full bg-[#c0c0c0]">
      ...
      <div className="flex items-center gap-1 p-1 border-b border-b-[#808080]">
        ...
        <button
          className={`win98-btn text-[11px] h-[22px] min-w-0 px-2 ${viewMode === 'list' ? 'border-[#404040] border-r-white border-b-white' : ''}`}
          onClick={() => setViewMode('list')}
        >
          <Icons.List size={12} />
        </button>
        <button
          className={`win98-btn text-[11px] h-[22px] min-w-0 px-2 ${viewMode === 'icons' ? 'border-[#404040] border-r-white border-b-white' : ''}`}
      ...
      <div className="flex flex-1 min-h-0">
        ...
        <div className="w-[140px] border-r border-r-[#808080] overflow-auto">
          <div className="p-1">
            <div
              className={`flex items-center gap-1 text-[11px] px-1 py-[2px] cursor-pointer ${currentPath.length === 0 ? 'bg-[#000080] text-white' : 'hover:bg-[#dfdfdf]'}`}
              onClick={() => { setCurrentPath([]); setSelectedItem(null); }}
            >
              ...
              <div key={child.name}>
                <div
                  className={`flex items-center gap-1 text-[11px] px-1 py-[2px] pl-4 cursor-pointer ${
                    currentPath.length === 1 && currentPath[0] === child.name ? 'bg-[#000080] text-white' : 'hover:bg-[#dfdfdf]'
                  }`}
                  onClick={() => {
                    if (child.type === 'folder') {
            ...
            <table className="w-full text-[11px]">
              ...
              <tbody>
                ...
                {children.map(child => (
                  <tr
                    key={child.name}
                    className={`cursor-pointer ${selectedItem === child.name ? 'bg-[#000080] text-white' : 'hover:bg-[#e8e8e8]'}`}
                  ...
                  >
                    ...
                    <td className="px-2 py-[2px]">{child.type === 'folder' ? 'Folder' : 'File'}</td>
                    <td className="px-2 py-[2px]">{child.modified || ''}</td>
                  </tr>
                ))}
                {children.length === 0 && (
                  <tr>
                    <td colSpan={4} className="px-2 py-4 text-center text-[#808080]">(Empty folder)</td>
                  </tr>
                )}
              </tbody>
            </table>
              ...
              {children.map(child => (
                <div
                  ...
                  className={`w-[72px] flex flex-col items-center gap-1 p-1 cursor-pointer ${
                    selectedItem === child.name ? 'bg-[#000080] text-white' : 'hover:bg-[#e8e8e8]'
                  }`}
                  onClick={() => setSelectedItem(child.name)}
                  onDoubleClick={() => navigateTo(child.name)}
                >
                  ...
                  <span className="text-[11px] text-center truncate w-full">{child.name}</span>
                </div>
              ))}
            </div>
          )}
